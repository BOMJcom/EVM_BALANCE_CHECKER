import { MULTICALL3_CONTRACT } from "../data/abis";
import { Chain } from "./interface";

export const RPC = "https://linea-mainnet.public.blastapi.io";

const USDC = "0x176211869ca2b568f2a7d4ee941e073a821ee1ff";
const USDT = "0xa219439258ca9da29e9cc4ce5596924745e12b93";
const DAI = "0x4af15ec2a0bd43db75dd04e62faa3b8ef36b00d5";
const WBTC = "0x3aab2285ddcddad8edf438c1bab47e1a9d05a9b4";
const WETH = "0xe5d7c2a44ffddf6b295a15c148167daaaf5cf34f";
const LXP = "0xd83af4fbd77f3ab65c3b1dc4b38d7e67aecf599a";

function getTokenList() {
    return [USDC, USDT, DAI, WBTC, WETH, LXP];
}

export const Linea: Chain = {rpc: RPC, name: 'Linea', MULTICALL3_CONTRACT: MULTICALL3_CONTRACT, tokenAddresses: getTokenList()};